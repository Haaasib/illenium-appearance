...
function client.rotateCamera(direction)
    ...
    local rotationDelta = (direction == "left" and -15.0 or 15.0)
    SetEntityHeading(playerPed, currentHeading + rotationDelta)
end

function client.adjustCameraHeight(delta)
    camHeight = math.max(-1.0, math.min(1.2, camHeight + delta))
end

function client.adjustCameraZoom(delta)
    camOffset = math.max(0.3, math.min(3.0, camOffset + delta))
end

local playerCoords
local function pedTurn(ped, angle)
    local currentHeading = GetEntityHeading(ped)
    SetEntityHeading(ped, currentHeading - angle)
end
...
local function wearClothes(data, typeClothes)
    ...
    local appliedComponents = data and data.components or {}
    local props = dataClothes.props[client.getPedDecorationType()]
    local appliedProps = data and data.props or {}

    if not appliedComponents then appliedComponents = {} end
    if not appliedProps then appliedProps = {} end
